...
:load_config
    :: 1. 读取配置文件（路径加双引号保护）
    for /f "tokens=1,2 delims== eol=#" %%a in ('type "script_config.txt"') do (
        set "%%a=%%b"
    )
...
:download_forge_installer
    ...
    :: PowerShell中URL的&符号需转义（已在load_config中用^转义）
...
:run_server
    ...
    :: 检查run.bat是否存在，并用call调用（避免路径问题）
    if exist "run.bat" (
        call "run.bat"
    ) else (
        echo [ERROR] 未找到run.bat，请确认脚本在服务器根目录下
        pause
    )
    exit /b 0
